...
// Use CORS-safe socket connection
...
async function fetchPeerId() {
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.once('token-resolved', (data) => {
        ...
        socket.once('error', () => reject(new Error("Socket error during peer ID resolution")));
    });
}
        ...
        peer.on('open', (receiverPeerId) => {
            ...
            conn.on('data', (data) => {
                if (data.fileName) {
                    ...
                    fileContainer.className = 'flex flex-col p-4 border border-white rounded-lg bg-[#2a2a2a] mb-4';
                    ...
                    file.content[data.chunkIndex] = Uint8Array.from(data.chunk);
